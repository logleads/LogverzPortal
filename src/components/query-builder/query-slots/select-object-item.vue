<template>
  <div ref="select" class="input-drop-down" contenteditable="true" disabled="true" @blur="handleBlur">
    ...
    <div v-if="scopedExpanded" :class="['input-unwrapped', { 'up-direction': direction }]" contenteditable="false">
      <div v-for="item in itemRef" :key="item.id + Math.random() * 10000" class="input-unwrapped__item"
        contenteditable="false" @click="handleClick($event, item)">
        {{ item.label }}
      </div>
    </div>
  </div>
